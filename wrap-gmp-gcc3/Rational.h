...
#define _POLYMAKE_GMP_RATIONAL_H "$Project: polymake $$Id: Rational.h,v 1.5 2003/01/27 09:24:46 bzframba Exp $"
...
/** Arbitrary precision rational number.
    ...
    Developed and tested with GMP Version 3.1.
...
private:
   ...
   void canonicalize() {
      if (!mpz_sgn(mpq_denref(rep))) throw gmp_error("Rational: zero denominator");
...
protected:
   ...
   template <proxy_kind kind, bool _canonicalize>
...
public:
   friend const proxy<num,true>& numerator(const Rational& a) {
      return *reinterpret_cast<const proxy<num,true>*>(mpq_numref(a.rep));
   }
   friend proxy<num,true>& numerator(Rational& a) {
      return *reinterpret_cast<proxy<num,true>*>(mpq_numref(a.rep));
   }
   friend const proxy<den,true>& denominator(const Rational& a) {
      return *reinterpret_cast<const proxy<den,true>*>(mpq_denref(a.rep));
   }
   friend proxy<den,true>& denominator(Rational& a) {
      return *reinterpret_cast<proxy<den,true>*>(mpq_denref(a.rep));
   }
   ...
   */
   ...
   friend bool abs_equal(const Rational& a, long b) {
     //////////////////////////////////////////////////////////////////////////////
     // for gcc < 3 we need to make std::abs(long) unique
     // 2003/01/27 jr:
     //       return !mpz_cmp_ui(mpq_denref(a.rep),1) &&
     // 	     mpz_fits_slong_p(mpq_numref(a.rep)) && mpz_get_si(mpq_numref(a.rep))==std::abs(b);
     // 
      return !mpz_cmp_ui(mpq_denref(a.rep),1) &&
	     mpz_fits_slong_p(mpq_numref(a.rep)) && mpz_get_si(mpq_numref(a.rep))==long(std::abs(Integer(b)));
      // end jr.
      //////////////////////////////////////////////////////////////////////////////
   }
   ...
   /** Input from a stream.
       Numerator and denominator should be separated by `/'.
       Omitted denominator is assumed equal to 1.
   ...
   */
   ...
   /** Output in a stream.
       ...
       The numeric base and its printable representation are chosen according to the
       \\ios::basefield\\ and \\ios::showbase\\ flags of the stream. Default base is 10.
   */
...
inline Rational operator/ (const Integer& a, const Rational& b) {
   if (!a && b) return Rational();
   const Integer g=gcd(numerator(b),a);
   if (g==1)
...
namespace std_ext {
   ...
   public:
      size_t operator() (const MP_RAT& a) const {
	 return _do(&a);
      }
   };
